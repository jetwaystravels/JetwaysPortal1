...
{
    public class RoundTripCommitBookingRT : Controller
    {
		...
		public async Task<IActionResult> RoundTripBookingViewRT()
        {
            ...
            using (HttpClient client = new HttpClient())
            {
                ...
                for (int k1 = 0; k1 < data.Airline.Count; k1++)
                {
                    ...
                    if (!string.IsNullOrEmpty(tokenview) && flagAirAsia == true && data.Airline[k1].ToLower().Contains("airasia"))
                    {
						...
						HttpResponseMessage responsepaynet = await client.PostAsync(AppUrlConstant.AirasiaPayment, content);
                        ...
                        if (responceCommit_Booking.IsSuccessStatusCode)
                        {
                            ...
                            var  blankcommitres = new { data = (object)null };
                            var commitresponce = JsonConvert.SerializeObject(blankcommitres, Formatting.Indented);
                           // logs.WriteLogsR( _responceCommit_Booking, "15-Commit_Booking_Res", "SameAirAsiaRT");
                            logs.WriteLogsR(commitresponce, "15-Commit_Booking_Res", "SameAirAsiaRT");
