...
namespace OnionArchitectureAPI.Services.Indigo
{
    public class _SellSSR : ControllerBase
    {
        ...
        // SameAirline Code below
        public async Task<IndigoBookingManager_.SellResponse> sellssrSameAirline(string Signature, AirAsiaTripResponceModel passeengerKeyList, List<string> ssrKey, List<string> BaggageSSrkey, List<string> FastfarwardAddon, List<string> PPBGAddon, bool Boolfastforward, int _a, string _Airline = "")
        {
            passengerscount = passeengerKeyList.passengerscount;
            SellResponse sellSsrResponse = null;
            using (HttpClient client = new HttpClient())
            {
                ...
                if (ssrKey.Count >= 0 || FastfarwardAddon.Count > 0 || BaggageSSrkey.Count > 0)
                {
                    ...
                    sellreqd.SellSSR.SSRRequest = new SSRRequest();
                    journeyscount = passeengerKeyList.journeys.Count;
                    if (journeyscount == 1) // For OneWay Sell SSR
                    {
                                                ...
                                                if (_obj.SSRffwOneWayI.Count > 0)
                                                {
                                                    for (int k = 0; k < PaxNum.Adults_.Count + PaxNum.Childs_.Count; k++)//Paxnum 1 adult,1 child,1 infant 2 meal
                                                    {
                                                        ...
                                                        if (baggagecount > 0 && k + 1 <= baggagecount)
                                                        {
                                                            ...
                                                            sellreqd.SellSSR.SSRRequest.SegmentSSRRequests[_id].PaxSSRs[idx].SSRNumber = Convert.ToInt16(0);
                                                            sellreqd.SellSSR.SSRRequest.SegmentSSRRequests[_id].PaxSSRs[idx].DepartureStation = passeengerKeyList.journeys[i].segments[j].designator.origin;
                                                            sellreqd.SellSSR.SSRRequest.SegmentSSRRequests[_id].PaxSSRs[idx].ArrivalStation = passeengerKeyList.journeys[i].segments[j].designator.destination;//FinaldestinationforSSR
                                                            //idx++;
                                                        }
                                                    }
                                                }
                    ...
                    {
                        logs.WriteLogsR(JsonConvert.SerializeObject(sellSsrRequest), "10-SellSSRReq", "IndigoRT");
                        logs.WriteLogsR(JsonConvert.SerializeObject(sellSsrResponse), "10-SellSSRRes", "IndigoRT");
        ...
        public async Task<IndigoBookingManager_.AssignSeatsResponse> AssignSeatSameAirline(string Signature, AirAsiaTripResponceModel passeengerKeyList, List<string> unitKey, int p, int keycount0, int keycount1, string _Airlineway = "")
        {
            journeyscount = passeengerKeyList.journeys.Count;
            AssignSeatsResponse _AssignseatRes = new AssignSeatsResponse();
            AssignSeatsRequest _AssignSeatReq = new AssignSeatsRequest();
